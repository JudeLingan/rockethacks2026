  ...
  return (
    <header style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background:"rgba(10,10,10,0.94)", backdropFilter:"blur(14px)", borderBottom:"1px solid var(--grey)", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px", height:54 }}>
      ...
      <nav style={{ display:"flex", gap:3 }}>
        ...
        <a href="https://github.com/JudeLingan/rockethacks2026/" target="_blank" rel="noreferrer" style={{ background:"transparent", border:"1px solid var(--grey)", color:"var(--grey-text)", fontSize:11, letterSpacing:"0.12em", padding:"6px 14px", cursor:"pointer", borderRadius:2, textDecoration:"none", display:"flex", alignItems:"center", gap:6 }}>
      ...
      <div style={{ textAlign:"center", maxWidth:720, position:"relative", zIndex:1 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:12, marginBottom:44 }}>
          {[
            ...
            { icon:"⌥", label:"SOURCE",  desc:"View on GitHub", action:()=>window.open("https://github.com/JudeLingan/rockethacks2026/","_blank") },
...
function ControlScreen() {
  ...
  // Keyboard → action name → sendCommand (which resolves to 1 byte)
  useEffect(() => {
    const km = {
    ...
    const dn = e => {
      const action = km[e.key];
      if (!action) return;
      e.preventDefault();
      if (!activeKeys.has(e.key)) {
        setActiveKeys(p => new Set([...p, e.key]));
        sendCommand(action);
      }
    };
    const up = e => {
      const action = km[e.key];
      if (!action) return;
      setActiveKeys(p => { const n = new Set(p); n.delete(e.key); return n; });
          ...
          {[
            ...
            <div key={label}>
              ...
              <input value={val} onChange={e=>setter(e.target.value)} placeholder={ph}
                style={{ width:"100%", background:"var(--surface2)", border:"1px solid var(--grey)", color:"var(--white)", fontSize:11, padding:"7px 10px", borderRadius:2, outline:"none" }}/>
            </div>
          ))}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ background:"var(--surface)", border:"1px solid var(--grey)", borderRadius:3, overflow:"hidden" }}>
            ...
            <div ref={logRef} style={{ height:260, overflowY:"auto", padding:"5px 0", fontSize:11 }}>
              {log.length === 0
                ? <div style={{ textAlign:"center", color:"var(--grey-text)", marginTop:34, fontSize:11 }}>No activity yet</div>
                : log.map((e,i) => (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              {Object.entries({forward:"F",backward:"B",left:"L",right:"R",stop:"S"}).map(([action, char]) => (
                <div key={action} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:10 }}>
                  <span style={{ color:"var(--grey-text)", letterSpacing:"0.1em" }}>{action.toUpperCase()}</span>
                  <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                    <span style={{ background:"var(--surface2)", border:"1px solid var(--grey)", color:"var(--white)", padding:"1px 7px", borderRadius:2, fontFamily:"monospace" }}>{char}</span>
                    <span style={{ color:"var(--grey-text)" }}>0x{char.charCodeAt(0).toString(16).toUpperCase()}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* E-Stop */}
          <button onMouseDown={()=>sendCommand("stop")}
            style={{ width:"100%", padding:13, background:"rgba(232,35,42,0.1)", border:"1px solid var(--red)", borderRadius:3, cursor:"pointer", fontSize:15, letterSpacing:"0.18em", color:"var(--red)", transition:"all 0.1s" }}
        ...
        <div style={{ marginBottom:64 }}>
          ...
          <div style={{ position:"relative", width:"100%", aspectRatio:"16/9", background:"var(--surface)", border:"1px solid var(--grey)", borderRadius:3, overflow:"hidden" }}>
            ...
            <iframe style={{ width:"100%", height:"100%", border:"none", display:"block" }}
              src="https://www.youtube.com/embed/fDU4EzS75yw"
